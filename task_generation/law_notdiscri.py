...
print("notdiscri", model_name, tag)
...
for i, law_name in tqdm(enumerate(qa_data)):
    ...
    for j, qa in enumerate(qas):
        question = LLMprompt.get_discri_prompt(qa["question"], prompt_type, [f"上一版本的{law_name}：{law}", f"最新版本的{law_name}：{law}"])
