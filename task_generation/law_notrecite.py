...
law_path = "../data/old_law_all.json"
law_qa_path = "../data/LawDiscriNOT.json"

root = "./result/notrecite"
...
print("notrecite", model_name, tag)
...
for i, law_name in tqdm(enumerate(qa_data)):
    law = law_data[law_name].strip()

    inputs = tokenizer(law, return_tensors='pt').input_ids
    _, length = inputs.shape
    
    
    question = LLMprompt.get_recite_prompt(law_name, prompt_type, [f"{law_name}：{law}"])
    
    if i == 0:
        print(question)
        
        
    answer = LLM.get_response(model, tokenizer, question, int(length*2))
    answers[law_name] = {
        "answer": answer,
        "ground_truth": law
    }
